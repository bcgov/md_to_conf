...
def check_page_edited(confluence, space, title):
    page = confluence.get_page_by_title(space, title)
    if not page:
        return False
    # Assuming the marker "<!-- Edited -->" is added to the page content if edited
    if "<!-- Edited -->" in page['body']['storage']['value']:
        return True
    return False

def create_or_update_page(confluence, space, title, content, parent_id=None):
    if check_page_edited(confluence, space, title):
        print(f"Page '{title}' has been edited. Skipping update.")
        return
    # Existing logic to create or update the page
    existing_page = confluence.get_page_by_title(space, title)
    if existing_page:
        confluence.update_page(existing_page['id'], title, content)
    else:
        confluence.create_page(space, title, content, parent_id=parent_id)

def update_page(page_id, title, html, version, ancestors, properties, attachments):
    # Construct the API request data here
